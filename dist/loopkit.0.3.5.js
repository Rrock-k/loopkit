...
(function(){
  ...
  function install(){
    ...
    style.textContent = `
      ...
      :host([data-lk-edge-position="top-notch"]) .edge {
        ...
        clip-path: path("M 0 0 H 58 C 78 0 94 5 110 19 C 125 33 139 42 160 42 H 172 C 193 42 207 33 222 19 C 238 5 254 0 274 0 H 292 V 0 H 0 Z");
      }
      :host([data-lk-edge-position="top-notch"]) .edge:hover,
      :host([data-lk-edge-position="top-notch"]) .edge:focus-visible {
        top: -22px !important;
      }
      :host([data-lk-edge-position="top-notch"]) .edge-label,
      :host([data-lk-edge-position="top-notch"]) .edge-shortcut {
      ...
      :host([data-lk-edge-position^="top-"]) .edge-shortcut { bottom:auto; top: calc(100% + 8px); }
      ...
      :host(.is-collapsed) .edge.is-dragging .edge-label {
  ...
  function positionPoint(edge, position){
    ...
    const notchTop = -30;
    const map = {
      'top-notch': { x: w / 2, y: notchTop + 46 / 2 },
  ...
  function nearestPosition(x, y){
    ...
    if (y <= 34 && x > w * .18 && x < w * .82) return 'top-notch';
